...
def train_agent(total_timesteps=1000, agent_strength=10, bandit_strength=6):
    """
    Train the PPO agent with a GRU-based feature extractor and track win rate.
    """    
    env = BattleEnv(agent_strength=agent_strength, bandit_strength=bandit_strength)
    ...
    try:
        with open('training_log.txt', 'w') as f:
            ...
            def callback(locals_, globals_):
                nonlocal wins, total_episodes
                done_array = locals_["dones"]
                obs_array = locals_["new_obs"]

                for i, done in enumerate(done_array):
                    if done:
                        total_episodes += 1
                        if obs_array[i][0] > 0:  # Agent wins if HP > 0
                            wins += 1

                if total_episodes > 0:
                    win_rate = (wins / total_episodes) * 100
                    print(f"Win Rate: {win_rate:.2f}% ({wins}/{total_episodes} wins)")
    ...
    model.save("graphic_rpg_model")
    print(f"Model saved as 'graphic_rpg_model'")
    print(f"Final Win Rate: {wins / total_episodes * 100:.2f}% ({wins}/{total_episodes} wins)")
